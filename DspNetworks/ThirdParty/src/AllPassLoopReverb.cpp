...
Compilation options: -lang cpp -rui -nvi -ct 1 -cn _AllPassLoopReverb -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
  ...
  public:
	...
	void fill_AllPassLoopReverbSIG0(int count, float* table) {
		for (int i1 = 0; i1 < count; i1 = i1 + 1) {
			...
			table[i1] = std::sin(9.58738e-05f * float(iRec52[0]));
			iVec6[1] = iVec6[0];
			iRec52[1] = iRec52[0];
		}
	}
...
class _AllPassLoopReverb final : public ::faust::dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -rui -nvi -ct 1 -cn _AllPassLoopReverb -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
	...
	void instanceConstants(int sample_rate) {
		...
		float fConst3 = std::tan(47123.89f / fConst0);
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow3 = fConst1 * std::max<float>(0.0f, std::min<float>(0.98f, float(fHslider1)));
		float fSlow4 = fConst1 * std::max<float>(0.0f, std::min<float>(1.0f, float(fHslider2)));
		float fSlow5 = fConst1 * std::max<float>(0.01f, std::min<float>(0.99f, float(fHslider3)));
		float fSlow6 = fConst1 * std::max<float>(-24.0f, std::min<float>(24.0f, float(fHslider4)));
		float fSlow7 = fConst1 * std::max<float>(-1.0f, std::min<float>(1.0f, float(fHslider5)));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp34 = 98.5f * (ftbl0_AllPassLoopReverbSIG0[std::max<int>(0, std::min<int>(int(65536.0f * fRec53[0]), 65535))] + 1.0f);
			...
			float fTemp42 = 165.5f * (ftbl0_AllPassLoopReverbSIG0[std::max<int>(0, std::min<int>(int(65536.0f * fRec54[0]), 65535))] + 1.0f);
			...
			float fTemp50 = 105.5f * (ftbl0_AllPassLoopReverbSIG0[std::max<int>(0, std::min<int>(int(65536.0f * fRec55[0]), 65535))] + 1.0f);
			...
			float fTemp56 = fRec15[1] + fRec16 + fTemp2 * fTemp55 - 0.65f * fRec13[1];
			fVec15[IOTA0 & 2047] = fTemp56;
			float fTemp57 = ((iTemp32) ? 0.0f : fConst9 + fRec56[1]);
			fRec56[0] = fTemp57 - std::floor(fTemp57);
			float fTemp58 = 156.5f * (ftbl0_AllPassLoopReverbSIG0[std::max<int>(0, std::min<int>(int(65536.0f * fRec56[0]), 65535))] + 1.0f);
			...
			fRec3[0] = fRec8;
			fRec4[0] = fRec9;
			output0[i0] = FAUSTFLOAT(fTemp1 + 0.5f * (fRec1[0] + fRec3[0]));
			output1[i0] = FAUSTFLOAT(fTemp1 + 0.5f * (fRec2[0] + fRec4[0]));
			iVec0[1] = iVec0[0];
			fRec0[1] = fRec0[0];
			fRec10[1] = fRec10[0];
			fRec17[1] = fRec17[0];
	...
	#define FAUST_COMPILATION_OPIONS "-lang cpp -rui -nvi -ct 1 -cn _AllPassLoopReverb -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
