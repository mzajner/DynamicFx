...
Compilation options: -lang cpp -rui -nvi -ct 1 -cn _mp3Compressor -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
...
class _mp3Compressor final : public ::faust::dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -rui -nvi -ct 1 -cn _mp3Compressor -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow1 = std::pow(1e+01f, 0.05f * std::max<float>(-96.0f, std::min<float>(96.0f, float(fHslider0))));
		float fSlow2 = 1.0f / std::max<float>(1.1920929e-07f, std::max<float>(1.0f, std::min<float>(2e+01f, float(fHslider1)))) + -1.0f;
		float fSlow3 = std::max<float>(fConst0, 0.001f * std::max<float>(1.0f, std::min<float>(3e+03f, float(fHslider2))));
		int iSlow4 = std::fabs(fSlow3) < 1.1920929e-07f;
		float fSlow5 = ((iSlow4) ? 0.0f : std::exp(-(fConst0 / ((iSlow4) ? 1.0f : fSlow3))));
		float fSlow6 = std::max<float>(fConst0, 0.001f * std::max<float>(1.0f, std::min<float>(1e+03f, float(fHslider3))));
		int iSlow7 = std::fabs(fSlow6) < 1.1920929e-07f;
		float fSlow8 = ((iSlow7) ? 0.0f : std::exp(-(fConst0 / ((iSlow7) ? 1.0f : fSlow6))));
		float fSlow9 = std::max<float>(-1e+02f, std::min<float>(1e+01f, float(fHslider4)));
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fHbargraph0 = FAUSTFLOAT(2e+01f * std::log10(std::max<float>(1.1754944e-38f, std::pow(1e+01f, 0.05f * fRec0[0]))));
			output0[i0] = FAUSTFLOAT(((iSlow0) ? fTemp0 : fSlow1 * fTemp4));
			...
			fHbargraph1 = FAUSTFLOAT(2e+01f * std::log10(std::max<float>(1.1754944e-38f, std::pow(1e+01f, 0.05f * fRec4[0]))));
			output1[i0] = FAUSTFLOAT(((iSlow0) ? fTemp7 : fSlow1 * fTemp11));
			fRec3[1] = fRec3[0];
			fRec2[1] = fRec2[0];
			fRec1[1] = fRec1[0];
			fRec0[1] = fRec0[0];
	...
	#define FAUST_COMPILATION_OPIONS "-lang cpp -rui -nvi -ct 1 -cn _mp3Compressor -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
