...
Compilation options: -lang cpp -rui -nvi -ct 1 -cn _GritAmp -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
...
class _GritAmp final : public ::faust::dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -rui -nvi -ct 1 -cn _GritAmp -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
	...
	void instanceConstants(int sample_rate) {
		...
		float fConst1 = 1.0f / std::tan(5403.5396f / fConst0);
	...
	void instanceResetUserInterface() {
		...
		fHslider3 = FAUSTFLOAT(7.2e+02f);
		fHslider4 = FAUSTFLOAT(-7.5f);
	}
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = 0.0005f * std::pow(1e+01f, 2.5f * std::max<float>(0.0f, std::min<float>(1.0f, float(fHslider0))));
		float fSlow1 = std::max<float>(0.0f, std::min<float>(1.0f, float(fHslider1))) + -1.0f;
		float fSlow2 = fConst7 * std::max<float>(0.0f, std::min<float>(1e+02f, float(fHslider2)));
		float fSlow3 = fConst7 * std::max<float>(1e+02f, std::min<float>(5e+03f, float(fHslider3)));
		float fSlow4 = fConst7 * std::max<float>(-15.0f, std::min<float>(0.0f, float(fHslider4)));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp1 = std::min<float>(1.0f, std::max<float>(0.0f, 1e+01f * fRec2[0] * std::pow(1e+01f, 0.005f * fRec5[0]) + 0.5f));
			fVec1[0] = fTemp1;
			fRec4[0] = -(fConst5 * (fConst6 * fRec4[1] - fConst4 * (fTemp1 - fVec1[1])));
			float fTemp2 = fRec2[0] * (fSlow1 * (std::min<float>(1.0f, std::max<float>(-1.0f, (_GritAmp_faustpower2_f(fRec4[0] + 0.5f) + -0.4f) * std::pow(1e+01f, 0.01f * fRec5[0]))) + 0.15f) + 1.0f);
			fVec2[0] = fTemp2;
			fRec6[0] = fSlow3 + fConst8 * fRec6[1];
			float fTemp3 = std::tan(fConst9 * fRec6[0]);
			float fTemp4 = 1.0f / fTemp3;
			float fTemp5 = 1.0f - fTemp4;
			float fTemp6 = fTemp4 + 1.0f;
			fRec1[0] = (1e+01f * (fTemp2 + fVec2[1]) - fTemp5 * fRec1[1]) / fTemp6;
			float fTemp7 = (fTemp4 + -1.0f) / fTemp3 + 1.0f;
			float fTemp8 = _GritAmp_faustpower2_f(fTemp3);
			float fTemp9 = 1.0f - 1.0f / fTemp8;
			float fTemp10 = (fTemp4 + 1.0f) / fTemp3 + 1.0f;
			fRec0[0] = fRec1[0] - (fTemp7 * fRec0[2] + 2.0f * fTemp9 * fRec0[1]) / fTemp10;
			fRec7[0] = fSlow4 + fConst8 * fRec7[1];
			fRec9[0] = (1e+01f * ((fTemp2 - fVec2[1]) / fTemp3) - fRec9[1] * fTemp5) / fTemp6;
			fRec8[0] = fRec9[0] - (fRec8[2] * fTemp7 + 2.0f * fRec8[1] * fTemp9) / fTemp10;
			output0[i0] = FAUSTFLOAT(fSlow0 * ((fRec0[0] + fRec0[2] + 2.0f * fRec0[1] + std::pow(1e+01f, 0.05f * (fRec7[0] + 7.5f)) * (fRec8[0] + fRec8[2] - 2.0f * fRec8[1]) / fTemp8) * std::pow(1e+01f, -0.005f * fRec5[0]) / fTemp10));
			fVec0[1] = fVec0[0];
			fRec3[1] = fRec3[0];
			fRec2[1] = fRec2[0];
			fRec5[1] = fRec5[0];
	...
	#define FAUST_COMPILATION_OPIONS "-lang cpp -rui -nvi -ct 1 -cn _GritAmp -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
