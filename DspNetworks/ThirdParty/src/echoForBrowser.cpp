...
Compilation options: -lang cpp -rui -nvi -ct 1 -cn _echoForBrowser -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
...
class _echoForBrowser final : public ::faust::dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -rui -nvi -ct 1 -cn _echoForBrowser -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
	...
	void instanceResetUserInterface() {
		...
		fVslider1 = FAUSTFLOAT(0.6f);
		fVslider2 = FAUSTFLOAT(1.0f);
		fVslider3 = FAUSTFLOAT(0.85f);
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = fConst2 * std::max<float>(0.0f, std::min<float>(1.0f, float(fVslider0)));
		float fSlow1 = fConst3 * std::max<float>(0.0f, std::min<float>(0.6f, float(fVslider1)));
		int iSlow2 = int(1.0f - std::max<float>(0.0f, std::min<float>(1.0f, float(fVslider2))));
		float fSlow3 = fConst3 * std::max<float>(0.0f, std::min<float>(0.95f, float(fVslider3)));
		float fSlow4 = 44.1f * std::max<float>(0.001f, std::min<float>(0.6816667f, float(fVslider4)));
		float fSlow5 = fConst3 * std::max<float>(0.0f, std::min<float>(1e+02f, float(fVslider5)));
		float fSlow6 = fConst3 * std::max<float>(-1.0f, std::min<float>(1.0f, float(fVslider6)));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec3[IOTA0 & 65535] = fTemp0 * (((iSlow2) ? 0.0f : float(input0[i0])) + fRec5[0] * fRec1[1]) + fTemp1 * fRec3[(IOTA0 - 1) & 65535];
			...
			output0[i0] = FAUSTFLOAT(fRec0[0] * fRec2);
			fRec15[IOTA0 & 65535] = fTemp0 * (((iSlow2) ? 0.0f : float(input1[i0])) + fRec5[0] * fRec13[1]) + fTemp1 * fRec15[(IOTA0 - 1) & 65535];
			...
			output1[i0] = FAUSTFLOAT(fRec0[0] * fRec14);
			fRec0[1] = fRec0[0];
			fRec4[1] = fRec4[0];
			fRec5[1] = fRec5[0];
			IOTA0 = IOTA0 + 1;
	...
	#define FAUST_COMPILATION_OPIONS "-lang cpp -rui -nvi -ct 1 -cn _echoForBrowser -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
