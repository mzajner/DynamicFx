...
Compilation options: -lang cpp -rui -nvi -ct 1 -cn _FlangerFeedback -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
  ...
  public:
	...
	void fill_FlangerFeedbackSIG0(int count, float* table) {
		for (int i1 = 0; i1 < count; i1 = i1 + 1) {
			iRec6[0] = iRec6[1] + 1;
			float fTemp5 = float(iRec6[0] + -1);
			float fTemp6 = 9.58738e-05f * fTemp5;
			float fTemp7 = 0.0001917476f * fTemp5;
			table[i1] = 0.31830987f * (float((0.0f <= fTemp6) & (fTemp6 <= 3.1415927f)) * (fTemp7 + -3.1415927f) + float((3.1415927f < fTemp6) & (fTemp6 <= 6.2831855f)) * (9.424778f - fTemp7));
			iRec6[1] = iRec6[0];
		}
	}
...
class _FlangerFeedback final : public ::faust::dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -rui -nvi -ct 1 -cn _FlangerFeedback -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow2 = fConst1 * std::max<float>(0.0f, std::min<float>(0.95f, float(fHslider1)));
		float fSlow3 = 0.001f * std::max<float>(0.0f, std::min<float>(0.05f, float(fHslider2)));
		float fSlow4 = fConst1 * std::max<float>(0.0f, std::min<float>(0.1f, float(fHslider3)));
		float fSlow5 = fConst1 * std::max<float>(0.0f, std::min<float>(1e+01f, float(fHslider4)));
		float fSlow6 = 0.5f * std::max<float>(0.0f, std::min<float>(1.0f, float(fHslider5)));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			output0[i0] = FAUSTFLOAT(fSlow1 * (fTemp0 + fSlow0 * fRec0[0]));
			...
			output1[i0] = FAUSTFLOAT(fSlow1 * (fTemp12 + fSlow0 * fRec7[0]));
			fRec1[1] = fRec1[0];
			IOTA0 = IOTA0 + 1;
			fRec2[1] = fRec2[0];
			fRec3[1] = fRec3[0];
	...
	#define FAUST_COMPILATION_OPIONS "-lang cpp -rui -nvi -ct 1 -cn _FlangerFeedback -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
