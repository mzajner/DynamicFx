...
Compilation options: -lang cpp -rui -nvi -ct 1 -cn _TransientShaper -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
...
class _TransientShaper final : public ::faust::dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -rui -nvi -ct 1 -cn _TransientShaper -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
	...
	void instanceConstants(int sample_rate) {
		...
		iConst7 = int(std::max<float>(0.0f, 0.03f * fConst0));
	}
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = fConst5 * std::max<float>(1e+01f, std::min<float>(2e+04f, float(fHslider0)));
		float fSlow1 = fConst5 * std::max<float>(2e+01f, std::min<float>(2e+04f, float(fHslider1)));
		float fSlow2 = fConst5 * std::pow(1e+01f, 0.05f * std::max<float>(-12.0f, std::min<float>(12.0f, float(fHslider2))));
		float fSlow3 = fConst5 * std::pow(1e+01f, 0.05f * std::max<float>(-12.0f, std::min<float>(12.0f, float(fHslider3))));
		...
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			fRec2[0] = fTemp0 - ((fRec3[2] + fRec3[0] + fTemp6) / fTemp5 + (fRec2[2] * fTemp9 + 2.0f * fRec2[1] * fTemp10) / fTemp11);
			float fTemp12 = 2.0f * fRec2[1];
			fRec1[IOTA0 & 8191] = fRec1[(IOTA0 - 1) & 8191] + _TransientShaper_faustpower2_f((fRec2[2] + fRec2[0] + fTemp12) / fTemp11);
			...
			fVbargraph0 = FAUSTFLOAT(float(iRec9[0]));
			int iTemp14 = fVbargraph0 != 1.0f;
			fRec10[0] = fTemp0 - (fTemp9 * fRec10[2] + 2.0f * fTemp10 * fRec10[1]) / fTemp11;
			output0[i0] = FAUSTFLOAT(fRec0[0] * (fRec2[0] + fRec2[2] + fTemp12) / fTemp11 + ((iTemp14) ? fTemp0 + (fRec3[0] + fRec3[2] + fTemp6) / fTemp5 - (fRec10[0] + fRec10[2] + 2.0f * fRec10[1]) / fTemp11 : 0.0f));
			float fTemp15 = float(input1[i0]);
			fRec14[0] = fTemp15 - (fTemp3 * fRec14[2] + 2.0f * fTemp4 * fRec14[1]) / fTemp5;
			float fTemp16 = 2.0f * fRec14[1];
			fRec13[0] = fTemp15 - ((fRec14[2] + fRec14[0] + fTemp16) / fTemp5 + (fTemp9 * fRec13[2] + 2.0f * fTemp10 * fRec13[1]) / fTemp11);
			float fTemp17 = 2.0f * fRec13[1];
			fRec12[IOTA0 & 8191] = fRec12[(IOTA0 - 1) & 8191] + _TransientShaper_faustpower2_f((fRec13[2] + fRec13[0] + fTemp17) / fTemp11);
			...
			output1[i0] = FAUSTFLOAT(fRec11[0] * (fRec13[0] + fRec13[2] + fTemp17) / fTemp11 + ((iTemp14) ? fTemp15 + (fRec14[0] + fRec14[2] + fTemp16) / fTemp5 - (fRec16[0] + fRec16[2] + 2.0f * fRec16[1]) / fTemp11 : 0.0f));
			IOTA0 = IOTA0 + 1;
			fRec4[1] = fRec4[0];
			fRec3[2] = fRec3[1];
			fRec3[1] = fRec3[0];
	...
	#define FAUST_COMPILATION_OPIONS "-lang cpp -rui -nvi -ct 1 -cn _TransientShaper -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
