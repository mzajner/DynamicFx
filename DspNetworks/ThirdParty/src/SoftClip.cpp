...
Compilation options: -lang cpp -rui -nvi -ct 1 -cn _SoftClip -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0
------------------------------------------------------------ */
...
class _SoftClip final : public ::faust::dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("compile_options", "-lang cpp -rui -nvi -ct 1 -cn _SoftClip -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0");
	...
	void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow0 = std::max<float>(0.0f, std::min<float>(1.0f, float(fEntry0)));
		...
		float fSlow3 = fConst1 * std::pow(1e+01f, std::max<float>(0.0f, std::min<float>(1.0f, float(fEntry1))));
		float fSlow4 = fConst1 * std::pow(1e+01f, std::max<float>(0.0f, std::min<float>(1.0f, float(fEntry2))));
		float fSlow5 = fSlow2 * std::atan(fSlow2);
		float fSlow6 = std::log(_SoftClip_faustpower2_f(fSlow2) + 1.0f);
		float fSlow7 = 1.0f - fSlow1;
		float fSlow8 = fConst1 * std::pow(1e+01f, -(0.15f * std::max<float>(0.0f, std::min<float>(1.0f, float(fEntry3)))));
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			...
			float fTemp4 = fSlow1 * (((std::fabs(fTemp2) <= fConst3) ? std::atan(0.5f * (fTemp1 + fVec1[1])) : (fSlow2 * fTemp0 * std::atan(fTemp1) - (fVec1[1] * std::atan(fVec1[1]) + 0.5f * (std::log(_SoftClip_faustpower2_f(fTemp1) + 1.0f) - std::log(_SoftClip_faustpower2_f(fVec1[1]) + 1.0f)))) / fTemp2) / ((std::fabs(fTemp3) <= fConst3) ? std::atan(0.5f * (fSlow2 + fVec0[1])) : (fSlow5 - (fVec0[1] * std::atan(fVec0[1]) + 0.5f * (fSlow6 - std::log(_SoftClip_faustpower2_f(fVec0[1]) + 1.0f)))) / fTemp3)) + fSlow7 * fTemp0;
			float fTemp5 = std::fabs(fTemp4);
			fRec2[0] = fSlow8 + fConst2 * fRec2[1];
			float fTemp6 = 1.0f - fRec2[0];
			float fTemp7 = std::min<float>(fTemp5, fRec2[0] + 0.63661975f * fTemp6 * std::atan(1.5707964f * ((fTemp5 - fRec2[0]) / fTemp6))) * float((fTemp4 > 0.0f) - (fTemp4 < 0.0f));
			fVec2[0] = fTemp7;
			float fTemp8 = _SoftClip_faustpower2_f(fTemp7 - fVec2[1]);
			int iTemp9 = (fTemp7 <= 1.0f) & (fTemp7 >= -1.0f);
			float fTemp10 = float((fTemp7 > 0.0f) - (fTemp7 < 0.0f));
			float fTemp11 = 0.5f * _SoftClip_faustpower2_f(fTemp7);
			int iTemp12 = (fVec2[1] <= 1.0f) & (fVec2[1] >= -1.0f);
			float fTemp13 = float((fVec2[1] > 0.0f) - (fVec2[1] < 0.0f));
	...
	#define FAUST_COMPILATION_OPIONS "-lang cpp -rui -nvi -ct 1 -cn _SoftClip -scn ::faust::dsp -es 1 -mcd 16 -mdd 1024 -mdy 33 -uim -single -ftz 0"
